...
if($dbOK){
    ...
    if($classi=$db->ExecQueryAssoc($query_classe)){
        ...
        if(isset($_GET['filterTag'])){
            ...
            $query_valutazione='SELECT Valutazione.nome_utente AS utente ,datav, commento, tag, p_complessivo, p_acc_fisica, p_servizio_inclusione, tempestivita_burocratica, p_insegnamento, Iscrizione.corso AS corso
            FROM Valutazione INNER JOIN Iscrizione ON Valutazione.nome_utente = Iscrizione.nome_utente
        ...
        if($valutazioni=$db->ExecQueryAssoc($query_valutazione)){
            ...
            foreach($valutazioni as $v){
                $contenuto.='<li id="commento"><strong>'.$v['utente']."|".$v['datav']." | ".$v['corso']."</strong><p id=testoCommento>".$v['commento']."</p>";
        ...
        if(!isset($_SESSION['user'])){
            ...
            $query_iscrizione='SELECT nome_utente FROM Iscrizione WHERE classe = '.$target.' AND nome_utente='.$_SESSION['user'].';';
